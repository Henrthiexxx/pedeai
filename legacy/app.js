...
try {
    ...
    console.warn('Firestore settings skipped:', err?.message || err);
}
...
async function publishStoreContext() {
    if (window.__PEDRA_WEB__) return; // web: sem servidor LAN, nada a publicar
...
async function loadStore() {
    try {
        let snap = currentUser?.email
            ? await db.collection('stores').where('ownerEmail', '==', currentUser.email).limit(1).get()
            : await db.collection('stores').where('ownerId', '==', currentUser.uid).limit(1).get();
        if (snap.empty && currentUser?.email) {
            snap = await db.collection('stores').where('adminEmails', 'array-contains', currentUser.email).limit(1).get();
        }
        if (snap.empty) {
            snap = await db.collection('stores').where('ownerId', '==', currentUser.uid).limit(1).get();
        }
        if (snap.empty) {
            snap = await db.collection('stores').where('userId', '==', currentUser.uid).limit(1).get();
        }
        if (snap.empty) {
            snap = await db.collection('stores').where('uid', '==', currentUser.uid).limit(1).get();
        }
        if (!snap.empty) {
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const canBootApp = await window.PedraPcUpdateGuard?.checkAndBlockIfNeeded({
        db,
        electronApi: window.PedraElectron
    });
    if (!canBootApp) {
        cleanup();
        return;
